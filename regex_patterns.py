...
ACCEPTABLE_REGEX = '^{prefix}?{delimiter}{date}{delimiter}{time}{delimiter}{suffix}?{extension}$'.format(
    **REGEX_PARTS
)
...
# ^(?P<prefix>IMG|VID|Screenshot|C360|MPC|DSC|RKP|MVI)?[-|_| |\.|~]?((?P<year>[1-2][9|0]\d{2})[-|_| |\.|~]?(?P<month>[0-1]\d)[-|_| |\.|~]?(?P<day>[0-3]\d))[-|_| |\.|~]?((?P<hour>[0-2]\d)[-|_| |\.|~]?(?P<minute>\d{2})[-|_| |\.|~]?(?P<second>\d{2})[-|_| |\.|~]?(?P<millisecond>\d{3})?)?[-|_| |\.|~]?(?P<suffix>\d{1,4}|\(\d\)|\w{1,10}|Burst\d{2}|WA\d{4})?\.(?P<extension>jpg|jpeg|png|mp4|mov|nef|gif|JPG|JPEG|PNG|MP4|MOV|NEF|GIF)$
